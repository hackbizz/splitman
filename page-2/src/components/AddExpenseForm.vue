<template>
  <v-dialog v-model="dialog" max-width="500">
      ...
      <v-card-text>
          <v-form ref="form">
          <v-text-field v-model="expenseName" label="Expense Name" required></v-text-field>
          <v-text-field v-model="expenseAmount" label="Expense Amount" required></v-text-field>
          <v-textarea v-model="expenseDescription" label="Expense Description" required></v-textarea>
          <v-file-input v-model="image" label="Bill Receipt" accept="image/*" required></v-file-input>
...
<v-col cols="12">
  ...
  <v-row v-for="(participant, index) in selectedParticipants" :key="index">
    <v-col cols="4">
              ...
              if (this.$refs.form.validate()) {
                if(this.splitType=="Equal"){
                  console.log("amount  hi");
                   this.selectedParticipantsWithAmount = this.selectedParticipants.map(id => {
                console.log("amount ",id);
return { user_id:id, amount: this.expenseAmount / this.selectedParticipants.length };
});
                }
                console.log("participant with amount ",this.selectedParticipantsWithAmount);
                  ...
                  formData.append('recipients',JSON.stringify(this.selectedParticipantsWithAmount) )
                  ...
                  // Example using axios:
                  await axios.post('http://localhost:3000/api/expenses/create', formData, { headers: { 'Content-Type': 'multipart/form-data' } })
                    .then(response => {
